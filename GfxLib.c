	...
	switch(strokes){
		case 0: //borders
			for(int angle=0; angle < nbr; angle++){
				ligne( x+R*cos(angle*2*Pi/div)    , y+R*sin(angle*2*Pi/div),
					   x+R*cos((angle+1)*2*Pi/div), y+R*sin((angle+1)*2*Pi/div) );
			}
		break;
		case 1: //center lines
			for(int angle=0; angle < nbr; angle++){
				ligne(x,y, x+R*cos(angle*2*Pi/div),y+R*sin(angle*2*Pi/div));
			}
		break;
